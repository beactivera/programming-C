...
void zapiszDoPliku(ksiazka ksiazki[], int pozycja);
void odczytajZPliku(ksiazka ksiazki[], int* pozycja);
...
int main(){
    ...
    odczytajZPliku(ksiazki, &pozycja);
    ...
    while(1) {
        ...
        if (cyfra >= 1 && cyfra <= 5){
            ...
            case 5 : zapiszDoPliku(ksiazki, pozycja); break;
            }
        }
        else {
...
void zapiszDoPliku(ksiazka ksiazki[], int pozycja) {
    ...
    for(int i=0; i<pozycja; i++){
        fprintf(plik,"%s ", ksiazki[i].title);
        fprintf(plik,"%s ", ksiazki[i].author);
        fprintf(plik,"%d ", ksiazki[i].year);
        fprintf(plik,"%d ", ksiazki[i].pages);
        fprintf(plik,"\n");
    }
   fclose(plik);
}
...
void odczytajZPliku(ksiazka ksiazki[], int* pozycja) {
    FILE* plik = plik = fopen("ksiazki.txt", "r");
    int iter = 0;
    char raz[20];
    char dwa[20];
    while(fscanf(plik, "%s%s%d%d", ksiazki[iter].title, ksiazki[iter].author, &ksiazki[iter].year, &ksiazki[iter].pages) == 4) {
        iter++;
        display(ksiazki+*pozycja);
        (*pozycja)++;
    }
...
void zapiszDoPlikuBinarnie(ksiazka ksiazki[], int pozycja) {
    FILE* plik = fopen("ksiazki_binarnie.bin", "wb");
    for(int i=0; i<pozycja; i++){
